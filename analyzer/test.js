...
*/
  ...
  ieee32.BinString = ieee32.Hex2Bin(obj)
  if (ieee32.BinString != "exit")
  {
    ieee32.Convert2Dec();
    if (ieee32.DispStr != "")
    {
      return ieee32.DispStr;
    }
    else
    {
      return ieee32.FullDecValue;
    }
  }

}

function intValue(arr_)
{
    return arr_[0]+(arr_[1]<<8)+(arr_[2]<<16)+(arr_[3]<<24);
}

$(function(){
    $('<style>table, tr, td {border: 1px solid 000;border-collapse: collapse;} td{width:78px;}span{font-size:14px;} .big{font-size:16px;font-weight:bold}</style>').appendTo('head');
    $(document.body).append('<div style="position: fixed; right: 0; background-color: #bbb; width: 400px;top: 0;padding: 2px;"><table style="width: 100%;background-color: #eee;padding: 2px;text-align: center;"><tbody><tr><td style="width:40px"></td><td><span id="hex-digit-1" class="big">FF</span></td><td><span id="hex-digit-2" class="big">FF</span></td><td><span id="hex-digit-3" class="big">FF</span></td><td><span id="hex-digit-4" class="big">FF</span></td></tr><tr><td class="big">Float</td><td colspan="4"><span id="float-value">FF</span></td></tr><tr><td class="big">Int</td><td colspan="4"><span id="int-value">FF</span></td></tr><tr><td rowspan="2" class="big">Short</td><td colspan="2"><span id="short-value-1">FF</span></td><td colspan="2"><span id="short-value-3">FF</span></td></tr><tr><td></td><td colspan="2"><span id="short-value-2">FF</span></td><td></td></tr><tr><td class="big">Char</td><td><span id="char-value-1">FF</span></td><td><span id="char-value-2">FF</span></td><td><span id="char-value-3">FF</span></td><td><span id="char-value-4">FF</span></td></tr><tr><td class="big">Bin</td><td><span id="bin-value-1">00000000</span></td><td><span id="bin-value-2">00000000</span></td><td><span id="bin-value-3">00000000</span></td><td><span id="bin-value-4">00000000</span></td></tr></tbody></table></div>');
    ...
    $(document.body).bind('mouseup', function(e){
        ...
        var hexString = selection.toString().replace(/\n/g,"").trim();
        var hexArray = (hexString != '' && hexString.split(" ")) || null;
        if (hexArray != null && hexArray.length != 0 && hexString.length < 15){
            var valueArray = new Array();
            
            for(var i = 0; i < hexArray.length/2; i++){
                var temp = hexArray[i];
                hexArray[i] = hexArray[hexArray.length-i-1];
                hexArray[hexArray.length-i-1] = temp;
            }
            for(var i = 0; i < 4; i++){
                if (i < hexArray.length)
                {
                    $("#hex-digit-"+(i+1)).text(hexArray[i]);
                    valueArray[i] = parseInt(hexArray[i], 16);
                }
                else
                {
                    $("#hex-digit-"+(i+1)).text('-');
            ...
            hexString = hexArray.toString().replace(/,/g,"");
            
            if (hexArray.length==4)
            {
                $('#float-value').text(compute(hexString));
                $('#int-value').text(intValue(valueArray));
                $('#short-value-1').text(valueArray[0]+(valueArray[1]<<8));
                $('#short-value-2').text(valueArray[1]+(valueArray[2]<<8));
                $('#short-value-3').text(valueArray[2]+(valueArray[3]<<8));
                $('#char-value-1').text(valueArray[0]);
                $('#char-value-2').text(valueArray[1]);
                $('#char-value-3').text(valueArray[2]);
                $('#char-value-4').text(valueArray[3]);
                $('#bin-value-1').text((valueArray[0]>=0&&valueArray[0].toString(2)||(~valueArray[0]).toString(2)));
                $('#bin-value-2').text((valueArray[1]>=0&&valueArray[1].toString(2)||(~valueArray[1]).toString(2)));
                $('#bin-value-3').text((valueArray[2]>=0&&valueArray[2].toString(2)||(~valueArray[2]).toString(2)));
                $('#bin-value-4').text((valueArray[3]>=0&&valueArray[3].toString(2)||(~valueArray[3]).toString(2)));
            }
            else if (hexArray.length==3)
            {
                ...
                $('#short-value-1').text(valueArray[0]+(valueArray[1]<<8));
                $('#short-value-2').text('-');
                $('#short-value-3').text(valueArray[1]+(valueArray[2]<<8));
                $('#char-value-1').text(valueArray[0]);
                $('#char-value-2').text(valueArray[1]);
                $('#char-value-3').text(valueArray[2]);
                $('#char-value-4').text('-');
            ...
            else if (hexArray.length == 2)
            {
                $('#float-value').text('-');
                $('#int-value').text('-');
                $('#short-value-1').text('-');
                $('#short-value-2').text(valueArray[0]+(valueArray[1]<<8));
                $('#short-value-3').text('-');
                $('#char-value-1').text('-');
                $('#char-value-2').text(valueArray[0]);
                $('#char-value-3').text(valueArray[1]);
                $('#char-value-4').text('-');
                $('#bin-value-1').text('-');
                $('#bin-value-2').text((valueArray[0]>=0&&valueArray[0].toString(2)||(~valueArray[0]).toString(2)));
                $('#bin-value-3').text((valueArray[1]>=0&&valueArray[1].toString(2)||(~valueArray[1]).toString(2)));
                $('#bin-value-4').text('-');
            }
            else
            {
